...
export default function ECO() {
  ...
  const handleFiles = (newFiles: File[] | FileList) => {
    ...
    setFiles(prev => {
      if (validFiles.length > 0) {
         if (validFiles.length > MAX_FILES || prev.length + validFiles.length > MAX_FILES) {
           showToast(`最多上传 ${MAX_FILES} 段录音，已替换新文件`);
         }
  ...
  const handleStart = () => {
    ...
    startTranscription(files, country, settingsAircraftType, settingsPersona, outputMode, cbtaReport, assessmentTarget, 'ECO');
  };
      ...
      {status === 'completed' ? (
        <header className="mb-8 mt-4 md:mt-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0 scroll-smooth custom-scrollbar">
             <button
               onClick={() => downloadAsPDF(resultData, outputMode, resultData.report_title ? `${resultData.report_title}.pdf` : 'Assessment_Report.pdf', 'ECO')}
             ...
             <button
               onClick={() => downloadAsWord(resultData, outputMode, resultData.report_title ? `${resultData.report_title}.docx` : 'Assessment_Report.docx', 'ECO')}
               className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-gray-200 hover:bg-gray-50 hover:border-gray-300 text-sm font-semibold text-gray-700 transition tracking-wide shadow-sm shrink-0"
             >
          ...
          >
            {resultData.report_markdown && (
              <motion.div 
                 ...
                 className="pt-2 px-4"
              >
                <div className="bg-white rounded-[32px] border border-gray-100 p-6 md:p-8 shadow-sm markdown-body min-h-[500px]">
                  <ReactMarkdown 
                    components={{
                      table: ({node, ...props}) => (
              ...
              <div className="flex items-center justify-between p-3.5 md:px-4 hover:bg-gray-50 active:bg-gray-100 transition-colors cursor-pointer" onClick={() => setCbtaReport(!cbtaReport)}>
                <div className="flex items-start gap-3.5 pr-4">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[17px] font-medium text-gray-900 tracking-tight leading-tight mb-1 mt-[4px]">ECO三维分析报告</span>
                    <span className="text-[13px] text-gray-500 leading-snug">生成基于 FOQA 的三维教员级评估（语速、标准用语、发音）</span>
                  </div>
                </div>
